...
# Function for pressing the keys of keyboard:
def Press(key):
    pyautogui.press(key);
...
while True:
    ...
    # converting BGR to HSV values:
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV);

    #setting the hsv value for Red and blue color:
    ...
    # creating masks for blue and red color:
    red_mask = cv2.inRange(hsv, lowred, highred);
    blue_mask = cv2.inRange(hsv, lowblue, highblue);

    # image/frame, start_point, end_point, color, thickness111177717
